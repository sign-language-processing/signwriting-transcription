...
def create_test_config(data_path="/output/poses", experiment_dir='/model/poses'):
    data_path = Path(data_path)
    experiment_dir = Path(experiment_dir)
    ...
    training:
        load_model: "{experiment_dir}/best.ckpt" # if uncommented, load a pre-trained model from this checkpoint
        random_seed: 42
        optimizer: "adam"
        normalization: "tokens"
        adam_betas: [0.9, 0.98] 
